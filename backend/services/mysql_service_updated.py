"""
Updated MySQL Service with R2 File Upload Integration
"""
...
# ============================================================
# CANDIDATE SERVICE (Updated with R2)
# ============================================================
...
class CandidateService(MySQLService):
    ...
    def upload_resume(self, user_id: str, file_content: bytes, filename: str) -> str:
        ...
        try:
            ...
            # Update profile
            self.upsert_profile(user_id, {"resume_url": resume_url})
    ...
    def upload_avatar(self, user_id: str, file_content: bytes, filename: str) -> str:
        ...
        try:
            # Upload to R2
            avatar_url = r2_service.upload_file(file_content, filename, "profilepics")
            
            # Update profile
            self.upsert_profile(user_id, {"avatar_url": avatar_url})
    ...
    def upload_intro_video(self, user_id: str, file_content: bytes, filename: str) -> str:
        """Upload intro video to R2 and update profile."""
        try:
            # Upload to R2
            video_url = r2_service.upload_file(file_content, filename, "videos")
            
            # Update profile
            self.upsert_profile(user_id, {"intro_video_url": video_url})
            
            return video_url
            
        except Exception as e:
            logger.error(f"Intro video upload failed: {str(e)}")
            raise Exception(f"Intro video upload failed: {str(e)}")
...
# ============================================================
# RECRUITER SERVICE (Updated with R2)
# ============================================================
...
class RecruiterService(MySQLService):
    ...
    def upload_company_logo(self, user_id: str, file_content: bytes, filename: str) -> str:
        """Upload company logo to R2 and update profile."""
        try:
            # Upload to R2
            logo_url = r2_service.upload_file(file_content, filename, "profilepics")
            
            # Update profile
            self.upsert_profile({"user_id": user_id, "avatar_url": logo_url})
            
            return logo_url
            
        except Exception as e:
            logger.error(f"Company logo upload failed: {str(e)}")
            raise Exception(f"Company logo upload failed: {str(e)}")
...
class VideoService(MySQLService):
    ...
    def upload_video(self, file_content: bytes, filename: str, folder: str = "videos") -> str:
        """Upload video to R2."""
        try:
            return r2_service.upload_file(file_content, filename, folder)
        except Exception as e:
