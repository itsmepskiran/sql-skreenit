...
async function handleRegistrationSubmit(e) {
    ...
    try {
        ...
        if (errorDetail.includes('user already exists') || errorDetail.includes('email already registered') || errorDetail.includes('already in use') || errorDetail.includes('duplicate')) {
            errorMessage = 'User already registered with this email. <a href="' + getLoginUrl() + '" style="color: #4f46e5; text-decoration: underline;">Click here to login</a> or <a href="' + CONFIG.PAGES.FORGOT_PASSWORD + '" style="color: #4f46e5; text-decoration: underline;">reset your password</a>.';
        } else if (errorDetail.includes('invalid email') || errorDetail.includes('email format')) {
            errorMessage = 'Please enter a valid email address.';
        } else if (errorDetail.includes('password too weak') || errorDetail.includes('weak password')) {
            errorMessage = 'Password is too weak. Please use at least 8 characters with a mix of letters, numbers, and symbols.';
        } else if (errorDetail.includes('missing required') || errorDetail.includes('required field')) {
            errorMessage = 'Please fill in all required fields.';
        } else if (errorDetail.includes('rate limit') || errorDetail.includes('too many requests')) {
            errorMessage = 'Too many registration attempts. Please try again after a few minutes.';
        }
